...
**Bitácoras (chofer)**
- `POST /api/bitacoras` — chofer llena su checklist matutino: `{ idVehiculo, odometroInicial, odometroImagen?, varillaAntes?, liquidoFrenos?, liquidoDireccion? }`
- `GET /api/bitacoras/activa` — el chofer consulta su bitácora de hoy
- `PATCH /api/bitacoras/:id/aprobar` — solo Admin, da el VoBo (pasa a `EnOperacion`)
- `PATCH /api/bitacoras/:id/cerrar` — chofer cierra su día: `{ odometroFinal, varillaDespues? }`
- `GET /api/bitacoras/:id/corte-caja` — ventas - abastecimientos - gastos + ingresos = total a entregar
- `GET /api/bitacoras` — solo Admin, lista con filtros `idVehiculo`/`fecha`
- `GET /api/bitacoras/:idBitacora/abastecimientos` · `POST .../abastecimientos` — carga de gas: `{ litros?, importe, ubicacion?, evidencia? }`
- `GET /api/bitacoras/:idBitacora/movimientos-extra` · `POST .../movimientos-extra` — gastos/ingresos libres: `{ tipo: "Gasto"|"Ingreso", concepto, importe, evidencia? }`

**Ubicación en tiempo real**
- `POST /api/ubicacion-actual` — el chofer manda su posición (cada 2 min desde la app): `{ idVehiculo, latitud, longitud }`. Solo se acepta si tiene una bitácora `EnOperacion` para ese vehículo hoy.
- `GET /api/ubicacion-actual` — para el mapa de Admin, posición actual de todos los vehículos

## Backend completo -- Bloques A a G

Con esto ya queda cubierto todo el backend base: catálogos, autenticación, inventario/kardex,
ventas con promociones, y operación de chofer (bitácoras, gas, gastos/ingresos, ubicación).
