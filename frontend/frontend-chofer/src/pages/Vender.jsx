...
import api, { urlImagen, subirImagen } from '../api/client';
...
export default function Vender() {
  ...
  function obtenerUbicacionActual() {
    return new Promise((resolve) => {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        () => resolve(null), // sin permiso o error de GPS -- no bloquea la venta
  ...
  async function cobrar() {
    ...
    try {
      ...
      const { data } = await api.post('/ventas', {
        ...
        detalle,
        pago: { importe: total, tipoPago, evidencia: evidencia || undefined },
      });
      setResultado(data);
      setCarrito({});
      setIdCliente('');
  ...
  return (
    <div>
      ...
      {tipoPago === 'Credito' && (
        <p style={{ fontSize: 12, color: 'var(--warning)', marginTop: -6, marginBottom: 14 }}>
          Solo funciona si el cliente tiene credito habilitado por Operacion.
        </p>
      )}
      ...
      <div className="total-bar">
        ...
        <button className="btn btn-brand" disabled={!idCliente || lineasCarrito.length === 0 || enviando || subiendoEvidencia} onClick={cobrar}>
