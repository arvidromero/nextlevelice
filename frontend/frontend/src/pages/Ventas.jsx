...
export default function Ventas() {
  ...
  async function onSeleccionarEvidencia(e) {
  ...
  async function registrarVenta(e) {
    ...
    try {
      ...
      const { data } = await api.post('/ventas', {
        ...
        idVehiculo,
        detalle,
        pago: { importe: subtotalEstimado, tipoPago, evidencia: evidencia || undefined },
      });
          ...
          <div className="card">
            ...
            <form onSubmit={registrarVenta}>
              ...
              {tipoPago === 'Transferencia' && (
                <div className="field">
                  <label>Evidencia de transferencia</label>
                  {evidencia && <p style={{ fontSize: 12, color: 'var(--success)', marginBottom: 6 }}>✓ Foto cargada</p>}
                  <input type="file" accept="image/*" onChange={onSeleccionarEvidencia} />
                  {subiendoEvidencia && <p style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>Subiendo...</p>}
                </div>
              )}
              {tipoPago === 'Credito' && (
                <p style={{ fontSize: 12, color: 'var(--warning)', marginTop: -8, marginBottom: 16 }}>
                  Esta venta se cargara al saldo de credito del cliente. Solo funciona si el cliente tiene credito habilitado.
                </p>
              )}
              ...
              <button className="btn btn-primary" style={{ width: '100%' }} disabled={guardando || subiendoEvidencia || lineasSinStock.length > 0}>
